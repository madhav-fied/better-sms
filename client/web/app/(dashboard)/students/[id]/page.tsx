...
import { getStudent, updateStudent, addParentGuardian, deleteParentGuardian } from '@/lib/api/students';
...
export default function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      const deletedPGIds = (s?.parent_guardians ?? [])
        .filter((pg) => !editExistingPGs.find((epg) => epg.id === pg.id))
        .map((pg) => pg.id);
      await Promise.all([
        ...deletedPGIds.map((pgId) => deleteParentGuardian(id, pgId)),
        ...editNewPGs
          .filter((pg) => pg.first_name.trim())
          .map((pg) => addParentGuardian(id, {
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {(s.parent_guardians?.length ?? 0) > 0 && (
          <div className="col-span-2">
            <Card title="Parents / Guardians">
              <div className="grid grid-cols-2 gap-4 mt-1">
                {s.parent_guardians!.map((pg) => (
                  <div key={pg.id} className="rounded-lg border bg-muted/20 p-3 space-y-1.5">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-semibold capitalize">{pg.relation}</span>
                      {pg.is_primary && (
                        <Badge variant="secondary" className="text-[10px] py-0">Primary</Badge>
                      )}
                    </div>
                    <Row label="Name" value={[pg.first_name, pg.last_name].filter(Boolean).join(' ') || pg.name} />
                    <Row label="Phone" value={pg.phone} />
                    <Row label="Email" value={pg.email} />
                    <Row label="Occupation" value={pg.occupation} />
                  </div>
                ))}
              </div>
            </Card>

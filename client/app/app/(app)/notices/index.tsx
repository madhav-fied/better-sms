...
export default function NoticesScreen() {
  ...
  // Teachers always send to class_sections; admins always send school_wide
  ...
  const createMutation = useMutation({
    mutationFn: () => createNotice(
      isAdmin
        ? { title, body: body || undefined, target_type: 'school_wide' }
        : { title, body: body || undefined, target_type: 'class_sections', target_class_section_ids: selectedClassIds }
    ),
    onSuccess: () => {
  ...
  const canSubmit = title.trim().length > 0 && (isAdmin || selectedClassIds.length > 0);
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
          <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }} keyboardShouldPersistTaps="handled">
              ...
              {/* Audience label — fixed per role, not selectable */}
              <View style={{ backgroundColor: '#f3f4f6', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 }}>
                <Text style={{ fontSize: 12, color: '#6b7280' }}>
                  Audience: <Text style={{ fontWeight: '600', color: '#374151' }}>
                    {isAdmin ? 'School Wide' : 'Your Class'}
                  </Text>
                </Text>
              </View>

              {/* Class picker for teachers */}
              {isTeacher && (
                <View>

...
Students → All Students
  → Table columns: Adm No, Name, Class, Gender, Father Name, Mobile, Status, Actions
  → Top filters bar:
      Name (text), Admission No (text), Mobile (text)
      Class Section (dropdown), Gender, Student Type, Admission Type
      Status (active/inactive), TC Generated (yes/no)
      DOB range (from/to)
  → "Search" button applies filters
  → "Reset" clears all filters
  → Pagination at bottom
  → Row actions: View, Edit, 360° View, Upload Docs
  → Bulk actions (checkbox select): Change Class, Deactivate, Generate TC
```
...
```
Query params:
  name?: str                   # searches first_name + last_name
  father_name?: str
  mother_name?: str
  mobile?: str                 # matches sms_mobile or parent mobile
  admission_no?: str
  student_type?: StudentType
  ...
  tc_generated?: bool
  class_section_id?: int
  search_mode?: Literal["exact","prefix","suffix","substr"]  # default: substr
  page: int = 1
  limit: int = 20
